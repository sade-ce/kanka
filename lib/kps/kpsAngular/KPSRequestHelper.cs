...
namespace kpsAngular
{
    public class KPSRequestHelper
    {
        ...
        /// <param name="password"></param>
        /// <returns></returns>
        public Patient KimlikNoIleAdresBilgisiSorgula(long kimlikNo, long sorgulayanKimlikNo, string userName, string password)
        {
            string token = GetSTSToken(sorgulayanKimlikNo, userName, password);
        ...
        /// <param name="password"></param>
        /// <returns></returns>
        public Patient BilesikKisiveAdresSorgula(long kimlikNo, long sorgulayanKimlikNo, string userName, string password)
        {
            string token = GetSTSToken(sorgulayanKimlikNo, userName, password);
            ...
            model.Address.Country = GetElementsByTagNameHelper(result, "Il");
            model.Address.CountryCode = GetElementsLongHelper(result, "IlKodu");
            model.Address.District = GetElementsByTagNameHelper(result, "Ilce");
            model.Address.DistricCode = GetElementsByTagNameHelper(result, "IlceKodu");
            model.Address.Village = GetElementsByTagNameHelper(result, "Koy");
 ...
 */            
        ...
        private string GetElementsByTagNameHelper(XmlDocument root, string tag)
        {
           ...
           private long GetElementsLongHelper(XmlDocument root, string tag)
        {
            var matches = root.GetElementsByTagName(tag);
            if (matches.Count > 0){
